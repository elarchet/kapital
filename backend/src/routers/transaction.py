...
from src.schemas.allocation import AllocationRead, AllocationSplitRequest
...
def read_transactions(
    ...
    position_id: Annotated[int | None, Query(description="Filter by a position ID")] = None,
    skip: Annotated[int, Query(ge=0)] = 0,
    limit: Annotated[int, Query(ge=1, le=1000)] = 100,
) -> list[RawTransaction]:
    """List raw transactions the user owns, optionally scoped to one position."""
    ...
    if position_id is not None:
        statement = statement.where(Allocation.position_id == position_id)
...
def split_transaction(
    ...
    """Replace a transaction's allocations with a new split across positions."""
    user_id = _require_user_id(current_user)
