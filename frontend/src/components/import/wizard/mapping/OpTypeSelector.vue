...
// ~2.2rem per pill row (one text line + padding + gap); rows cap at 18vh.
const pillRows = useChunkedRows(() => props.variants, 2.2, 0.18);
</script>

<template>
  <div>
    <!-- Same layout rule as the CSV column tray: pills pack back-to-back over
         at least 2 rows (more when the screen is tall enough) and overflow
         scrolls horizontally. -->
    <div class="flex flex-col items-start gap-1.5 overflow-x-auto overflow-y-hidden pb-0.5">
      <div v-for="(row, rowIdx) in pillRows" :key="rowIdx" class="flex items-center gap-1.5 w-max">
        <button
