...
export function parsePreviewRows(params: {
  ...
  const getColumnConfigForField = (fieldKey: string, rawAction: string, opType: string) => {
  ...
  return previewLines.map(line => {
    ...
    const applyTrans = (dbKey: string, rawVal: string) => {
      ...
      const idx = getMappedColIdxForField(dbKey, rawAction, opType, params.columnConfigMap, params.uiColumns);
      if (idx !== -1) {
        const conf = getColumnConfigForField(dbKey, rawAction, opType) as ColMapping | null;
    ...
    if (feeAmtVal && parseFloat(feeAmtVal) > 0) {
      ...
      if (rawFeeType) {
        ...
        if (idx !== -1) {
          const conf = getColumnConfigForField('fee_type', rawAction, opType) as ColMapping | null;
    ...
    const txConf = getColumnConfigForField('transaction_id', rawAction, opType) as ColMapping | null;
